    ...
    serialPort.open(QIODevice::ReadOnly);
     ...
     QObject::connect(&serialPort, &QSerialPort::readyRead,[&]
     {
        ...
        for (char byte : rawData) {
            hexData += QString::asprintf("%02X", static_cast<unsigned char>(byte)).toUtf8();
        }

        hexData = hexData.toUpper();
        hexData=hexData.simplified(); // Remove spaces
        ui->plainTextEdit_2->appendPlainText(hexData);





                       QString inputText = ui->plainTextEdit_2->toPlainText().toUpper();
...
void MainWindow::format()
{
    ...
    for (int row = 0; row < patterns.size(); ++row) {
        ...
        if (sensorType == "8040") {
            ...
            formattedF = QString::number(f, 'f', 8);
            formattedF1 = QString::number(f1, 'f', 8);
            formattedF2 = QString::number(f2, 'f', 8);
            ...
            formattedG = QString::number(g, 'f', 8);
            formattedG1 = QString::number(g1, 'f', 8);
            formattedG2 = QString::number(g2, 'f', 8);
    ...
    if (!filePath.isEmpty()) {
        ...
        if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
            ...
            while (!in.atEnd()) {
                ...
                ui->plainTextEdit_2->insertPlainText(chunk);
                qApp->processEvents(); // Process events to keep GUI responsive

                // Optionally, you can add a progress indicator
                double progress = static_cast<double>(totalRead) / file.size();
                // Update your progress indicator (e.g., a QProgressBar)
            }
